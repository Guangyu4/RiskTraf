...
def evaluate(model, val_loader, mean, std, device, model_name, horizons=[3, 6, 12]):
    ...
    for horizon in horizons:
        if horizon <= preds_flow.shape[1]:
            pred_h = preds_flow[:, horizon-1:horizon, :, :]
            label_h = labels_flow[:, horizon-1:horizon, :, :]
    ...
    overall_mae = masked_mae(preds_flow, labels_flow).item()
    overall_rmse = masked_rmse(preds_flow, labels_flow).item()
    overall_mape = masked_mape(preds_flow, labels_flow).item()
    results['overall'] = {'mae': overall_mae, 'rmse': overall_rmse, 'mape': overall_mape}
    
    return results


def get_model(model_name, num_nodes, adj_path, device, in_steps=12, out_steps=12, in_dim=3):
    if model_name == 'STGCN':
        model = STGCN(
            n_vertex=num_nodes,
            adj_path=adj_path,
            Kt=3,
            Ks=3,
            blocks=[[in_dim], [64, 16, 64], [64, 16, 64], [128, 128], [out_steps]],
            T=in_steps,
            act_func="glu",
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='./checkpoints_3dim')
    parser.add_argument('--corr_threshold', type=float, default=0.3)
    parser.add_argument('--early_stop_patience', type=int, default=10)
    
    args = parser.parse_args()
    
    log_dir = f'./logs_3dim/{args.dataset}'
    ...
    if not os.path.exists(adj_path):
        logging.info(f"Building Pearson correlation adjacency matrix...")
    ...
    logging.info(f'Dataset: {args.dataset}, Model: {args.model} (3-dim input)')
    logging.info(f'Num nodes: {num_nodes}, Input dim: 3 (flow, speed, occupancy)')
    logging.info(f'Train samples: {len(train_loader.dataset)}')
    logging.info(f'Val samples: {len(val_loader.dataset)}')
    logging.info(f'Test samples: {len(test_loader.dataset)}')
    
    model = get_model(args.model, num_nodes, adj_path, args.device, args.in_steps, args.out_steps, in_dim=3)
    ...
    logging.info(f'\n{"="*60}')
    logging.info(f'Test Results: {args.dataset} - {args.model} (3-dim)')
    logging.info(f'{"="*60}')
